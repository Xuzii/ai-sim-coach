"""The iRacing telemetry type system and its mapping to Python / struct format chars.

PORT of the visualiser's ``rtv/catalog/types.py`` -- kept near-verbatim so the two
projects reconcile trivially. The only deliberate divergence: ``StorageStrategy`` uses
``(str, Enum)`` rather than ``StrEnum`` (3.11+) because pitwall supports Python 3.10.

Every telemetry variable has exactly one of six wire types (``irsdk_VarType``). pitwall
only needs ``IRType`` + ``mapping_for().struct_char`` to unpack the ``.ibt`` sample
buffer; the rest of the table rides along to keep the diff against ``rtv`` minimal.
"""
